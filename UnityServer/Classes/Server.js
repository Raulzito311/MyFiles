...
module.exports = class Server {
    ...
    //Interval updates every 100 miliseconds
    onUpdate() {
        let server = this;
    ...
    onDisconnected(connection = Connection) {
        ...
        if(server.lobbys[currentLobbyIndex].id != 0) {
            if(server.lobbys[currentLobbyIndex].connections.length == 0) {
    ...
    onAttemptToJoinGame(connection = Connection) {
        // Look throgh lobbies for a gamelobby
        // Check if joinable
